...
const BATCH_INPUT_PATH = path.resolve('openai-batches/localize-books/');
const OUTPUT_PATH = path.resolve('openai-batches/localize-books-output/');
...
const books = await getBooksData({ populateAuthor: true });
...
if (task === 'check') {
  ...
  for (const batchFileName of filesInPath) {
    ...
    const results: Record<string, Record<string, { primaryName?: string }>> = {};
    let errors = '';
    ...
    outputData.forEach(response => {
      ...
      try {
        ...
        console.log('Failed to parse: ' + customId);
        errors += `${customId}: \n` + content + '\n';
      }
    });
...
// task === 'create'
const batch = books.flatMap(book => {
  const result = [];
  ...
  for (const language of languages) {
    ...
    result.push({
      custom_id: `${book.id}:${language.code}:name`,
      method: 'POST',
  ...
  "primaryName": "..."
          ...
          },
          {
            ...
            content: JSON.stringify({
              ...
              author: {
                primaryArabicName: book.author?.primaryArabicName,
                primaryLatinName: book.author?.primaryLatinName,
                otherArabicNames: book.author?.otherArabicNames,
                otherLatinNames: book.author?.otherLatinNames,
              },
            }),
          },
        ],
